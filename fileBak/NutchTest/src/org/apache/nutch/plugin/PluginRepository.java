...
public class PluginRepository {
  ...
  private HashMap<String, ExtensionPoint> fExtensionPoints;//installExtensionPoints()�����������
  ...
  public PluginRepository(Configuration conf) throws RuntimeException {
    ...
    //conf.set("query.title.boost", "0");
    String urlsboost = conf.get("query.title.boost");//test
    ...
    String excludesplug = conf.get("plugin.excludes", "");
    String includesplug = conf.get("plugin.includes", "");
    Pattern excludes = Pattern.compile(excludesplug);
    Pattern includes = Pattern.compile(includesplug);
  ...
  /**
   * @return a cached instance of the plugin repository,�����ʵ��
   */
  ...
  private void displayStatus() {
    ...
    if ((fRegisteredPlugins == null) || (fRegisteredPlugins.size() == 0)) {
      ...
      for (PluginDescriptor plugin : fRegisteredPlugins) {
        LOG.info("\t" + plugin.getName() + " (" + plugin.getPluginId() + ")");System.out.println("\t" + plugin.getName() + " (" + plugin.getPluginId() + ")");
      }
    }
    ...
    if ((fExtensionPoints == null) || (fExtensionPoints.size() == 0)) {
      ...
      for (ExtensionPoint ep : fExtensionPoints.values()) {
        LOG.info("\t" + ep.getName() + " (" + ep.getId() + ")");System.out.println("\t" + ep.getName() + " (" + ep.getId() + ")");
      }
    }
  }
