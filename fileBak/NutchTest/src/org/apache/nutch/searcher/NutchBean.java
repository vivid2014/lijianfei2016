...
//  }
  ...
  public NutchBean(Configuration conf, Path dir) throws IOException {
    ...
    if (fs.exists(luceneConfig) || fs.exists(solrConfig)) {
      ...
      searchBean = new LuceneSearchBean(conf, indexDir, indexesDir);//创建searchBean对象
    }
    ...
    if (fs.exists(segmentConfig)) {
      ...
      segmentBean = new FetchedSegments(conf, new Path(dir, "segments"));//创建segmentBean对象
    }

    linkDb = new LinkDbInlinks(fs, new Path(dir, "linkdb"), conf);//创建linkDb对象
  }
...
//����Ķ�ûִ��
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      final Query query = Query.parse(args[0], conf);//把查询转化为一个lucene查询
      query.getParams().setMaxHitsPerDup(30);
      final Hits hits = bean.search(query);
      System.out.println("Total hits: " + hits.getTotal());
      final int length = (int)Math.min(hits.getLength(), 100);
      ...
      for (int i = 0; i < hits.getLength(); i++) {
        //System.out.println(" " + i + " " + details[i] + "\n" + summaries[i]);
    	  Hit hit = show[i]; 
    	  HitDetails hitD = details[i];
    	 // String sumry = bean.getParseText(hitD).getText();
    	  //System.out.println("网页文本:"+sumry);
    	  System.out.println("第"+(i+1)+"条数据:  标题---"+ details[i].getValue("title")+";"
    			  +"地址---"+details[i].getValue("url")+"摘要---"+summaries[i].toString());
