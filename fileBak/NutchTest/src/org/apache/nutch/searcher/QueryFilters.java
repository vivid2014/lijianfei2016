...
public class QueryFilters {
  ...
  private QueryFilter[] queryFilters;//查询过滤
...
//查询过滤
  public QueryFilters(Configuration conf) {
    ...
    this.queryFilters = (QueryFilter[]) objectCache.getObject(QueryFilter.class.getName());//获取查处过滤
    if (this.queryFilters == null) {
      try {
        ExtensionPoint point = PluginRepository.get(conf)
            .getExtensionPoint(QueryFilter.X_POINT_ID);//QueryFilter这个是一个扩展点
        if (point == null)
  ...
  public BooleanQuery filter(Query input) throws QueryException {
    ...
    //input: ��  ý��
    //output:
    //+(url:��^4.0 anchor:��^2.0 content:�� title:��^1.5 host:��^2.0) +
    //(url:ý^4.0 anchor:ý^2.0 content:ý title:ý^1.5 host:ý^2.0) 
    //url:"�� ý"~5^4.0 anchor:"�� ý"~4^2.0 content:"�� ý"~5 title:"�� ý"~5^1.5 host:"�� ý"~5^2.0
