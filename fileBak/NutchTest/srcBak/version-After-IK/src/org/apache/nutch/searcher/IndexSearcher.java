...
public class IndexSearcher implements Searcher, HitDetailer {
  ...
  public IndexSearcher(Path index,  Configuration conf)
    throws IOException {
    this.conf = conf;
    this.fs = FileSystem.get(conf);//下面的IndexReader.open(Directory directory)函数是在lucene-core-3.0.1中实现的
    init(IndexReader.open(getDirectory(index)), conf);//根据这个index初始化(init)一个IndexSearcher
  }

  private void init(IndexReader reader, Configuration conf) throws IOException {
    this.reader = reader;
    this.luceneSearcher = new org.apache.lucene.search.IndexSearcher(reader);//IndexSearcher中包含一个luceneSearcher对象
    this.luceneSearcher.setSimilarity(new NutchSimilarity());//lucene-core-3.0.1中实现的
    this.optimizer = new LuceneQueryOptimizer(conf);//查询优化，lucene-core-3.0.1中实现的
    this.queryFilters = new QueryFilters(conf);//查询过滤器
  }
  ...
  public Hits search(Query query) throws IOException {
    org.apache.lucene.search.BooleanQuery luceneQuery =//这里将查询转化为一个lucene查询
  ...
  private Hits translateHits(TopDocs topDocs,
    ...
    String[] dedupValues = null;//dedupValues存放所有爬去到的网页的地址？？？
    if (dedupField != null) 
      dedupValues = FieldCache.DEFAULT.getStrings(reader, dedupField);

    ScoreDoc[] scoreDocs = topDocs.scoreDocs;//doc排名前十的网页doc=231 score=0.13520202, doc=200 score=0.11608745。。。。。。。。。。
