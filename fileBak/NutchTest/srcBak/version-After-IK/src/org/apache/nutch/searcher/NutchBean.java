...
//下面的都没执行
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      final Query query = Query.parse(args[0], conf);//就是把输入的查询语句分析了一下，返回的还是这个查询语句的一个Query对象
      ...
      for (int i = 0; i < hits.getLength(); i++) {
        //System.out.println(" " + i + " " + details[i] + "\n" + summaries[i]);
    	  System.out.println("第"+(i+1)+"条:  标题---"+ details[i].getValue("title")+";"
    			  +"地址---"+details[i].getValue("url")+"总结---"+summaries[i].toString());
      }
    } catch (Throwable t) {
